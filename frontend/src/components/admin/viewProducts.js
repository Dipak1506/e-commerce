...
const ViewProducts = ({ setShowLoading }) => {
  ...
  const fetchProducts = async () => {
    api
      .getApi(`${process.env.REACT_APP_BACKEND_URL}products`)
                  ...
                  <span
                    onClick={() => {
                      if (
                        ...
                        api
                          .getApi(
                            `${process.env.REACT_APP_BACKEND_URL}/delete-product/${product.id}`
                          )
                          .then((response) => response.json())
                          .then((result) => {
                          ...
                          .catch((err) => {
                            //setShowLoading(false);
                            alert(
                              "Action failed. Product could not be deleted."
                            );
                          });
                      }
                    }}
